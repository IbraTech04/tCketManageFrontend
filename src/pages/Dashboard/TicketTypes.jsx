...
export default function TicketTypes() {
  ...
  function patchEventZones(newZones) {
    setZones(newZones);
    if (currentEvent) setCurrentEvent({ ...currentEvent, zones: newZones });
  }
  ...
  async function handleAddZone() {
    ...
    try {
      ...
      patchEventZones([...zones, zone]);
      setMatrix((prev) => {
        const next = { ...prev };
        for (const ttId of Object.keys(next)) {
          next[ttId] = { ...next[ttId], [zone.id]: undefined };
        }
        return next;
      });
      setNewZoneName('');
  ...
  async function handleDeleteZone(zoneId) {
    ...
    try {
      ...
      patchEventZones(zones.filter((z) => z.id !== zoneId));
      setMatrix((prev) => {
        const next = {};
        for (const [ttId, cols] of Object.entries(prev)) {
          const { [zoneId]: _removed, ...rest } = cols;
          next[ttId] = rest;
        }
        return next;
      });
                ...
                {isEditing ? (
                  <>
                    <input
                      ...
                      autoFocus
                    />
                    <Button variant="primary" size="sm" onClick={() => handleSaveZoneEdit(zone.id)} disabled={zoneBusy || !editZoneName.trim()}>
